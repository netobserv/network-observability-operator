...
SOURCE=docs/fields.yaml
FILTERS_SOURCE=controllers/consoleplugin/config/static-frontend-config.yaml
...
echo -e '[cols="1,3,1,1",options="header"]' >> $ADOC
echo -e '|===' >> $ADOC
echo -e '| Name | Description | Filter ID | Loki label' >> $ADOC
...
for i in $(seq 0 $(( $nbfields-1 )) ); do
  ...
  if [[ "$filter" == "null" ]]; then
    filter=$(yq ".columns[] | select(.field == \"$name\").filter" $FILTERS_SOURCE | sed 's/null//')
    if [[ "$filter" == "" ]]; then
      filter="n/a"
    fi
  fi
