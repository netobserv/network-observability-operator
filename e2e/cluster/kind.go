...
var log = tester.InitLogger("component", "cluster.Kind")
...
func (k *Kind) Run(m *testing.M) {
	...
	log.Info("starting kind setup", "cluster name", k.clusterName)
	code := k.testEnv.Setup(envFuncs...).
		Finish(
			k.exportLogs(),
			envfuncs.DestroyKindCluster(k.clusterName),
		).Run(m)
	log.Info("tests finished run", "returnCode", code)
}
...
func (k *Kind) exportLogs() env.Func {
	return func(ctx context.Context, config *envconf.Config) (context.Context, error) {
		...
		log.Info("exporting cluster logs", "directory", logsDir)
		exe := gexe.New()
		out := exe.Run("kind export logs " + logsDir + " --name " + k.clusterName)
		log.Info("exported cluster logs", "out", out)
		return ctx, nil
	}
}
...
func deployManifestFile(definition Deployment,
	...
	log.Info("deploying manifest file", "file", definition.ManifestFile)
	...
	for {
		...
		if _, err := dri.Create(context.Background(), unstructuredObj, metav1.CreateOptions{}); err != nil {
			log.Error(err, "Cannot create resource", "manifest", definition.ManifestFile)
			panic(err)
		}
	}
}
...
func withTimeout(f env.Func, timeout time.Duration) env.Func {
	tlog := log.WithValues("function", "withTimeout")
	return func(ctx context.Context, config *envconf.Config) (context.Context, error) {
		start := time.Now()
		for {
			...
			tlog.WithValues("error", err).Info("function did not succeed. Retrying after 1s")
...
func isReady(definition Deployment) env.Func {
	return func(ctx context.Context, cfg *envconf.Config) (context.Context, error) {
		if definition.ReadyFunction != nil {
			log.WithValues("function", "isReady", "deployment", definition.ManifestFile).
