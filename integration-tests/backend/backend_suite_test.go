...
// Output path is controlled by JUNIT_REPORT_FILE env var. Do not use --junit-report flag,
// as ginkgo's autogenerated reporter runs after this hook and would overwrite with the full unfiltered report.
var _ = g.ReportAfterSuite("NetObserv JUnit", func(report g.Report) {
	...
	if err := reporters.GenerateJUnitReport(filtered, junitFile); err != nil {
		g.Fail(fmt.Sprintf("failed to write filtered JUnit report to %s: %v", junitFile, err))
	}
})
